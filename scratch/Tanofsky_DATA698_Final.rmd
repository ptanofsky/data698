...
The model inputs mimic a map-based application in which a user selects a point on a map along with the type of day and time to receive a prediction of the bikes available at that future date, time, and location. Based on the user-provided longitude and latitude, a nearest neighbor search is performed to identify the nearest docking station. This translation of the longitude and latitude to the corresponding cluster of the nearest-neighbor docking station is be the input to the model. The categorical variable type of day, weekday or weekend, was chosen due to the differing usage patterns on weekdays and weekends. Within each category, weekday and weekend, the similarities allowed us to simplify the model to just two values instead of accounting for each day separately. The time of day is converted to the start of the hour. Models were constructed using the 15-minute intervals, but due to computing system constraints the time interval of one hour was selected. The dependent variable is the count of available bikes with the floor function applied to ensure a whole number prediction.
