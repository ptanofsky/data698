...
v3: Attempt to cleaning up in preparation for presentation and final report
...
# Final table for 15M interval for all BK clusters

```{r}
rmse_15M <- c(rmsemodelp,rmsemodelqp,rmsemodelnb)
mae_15M <- c(maemodelp,maemodelqp,maemodelnb)
...
```{r}
rmse_1H <- c(rmsemodelp,rmsemodelqp,rmsemodelnb,
             rmsemodelhp,rmsemodelhnb,
             rmsemodelzp,rmsemodelznb)
mae_1H <- c(maemodelp,maemodelqp,maemodelnb,
...
paletteBins <- c(0, 25, 50, 75, 100, 125, 150, 200, 500)
...
addLegend(pal = colorPalette, values = bk_stations_bike_availZero_counts$count, opacity=0.9, title = "Frequency of Zero Availability", position = "topright")
...
```{r}
mapshot(m_bk_zeroAvail, file = "bk_stations_zero_avail.jpg")
```

# Now to try to identify rebalancing for BK only, all of NYC attempted below
...
# Now to try to identify rebalancing for all of NYC (copied from above)

```{r}
station_ids <- stations_attrs_trim$station.id

columns_nyc <- append("timestamp", station_ids)
columns_nyc
```
...
cols_nyc_int <- intersect(columns_all, columns_nyc)


nyc_stations_bikeAvail_15M <- stations_with_bike_avail[,cols_nyc_int]
```


```{r}
nyc_stations_bikeAvail_15M
```
...
# calculating rows of dataframe
rows <- nrow(nyc_stations_bikeAvail_15M)
  
# difference in rows of entire dataframe
nyc_stations_bikeAvail_15M_diff <- nyc_stations_bikeAvail_15M[-1,-1] - nyc_stations_bikeAvail_15M[-rows,-1]
  
nyc_stations_bikeAvail_15M_diff
```

```{r}
nyc_stations_15M_interval_diff <- cbind(nyc_stations_bikeAvail_15M[-1,1], nyc_stations_bikeAvail_15M_diff) %>%
  rename(timestamp=`nyc_stations_bikeAvail_15M[-1, 1]`)

nyc_stations_15M_interval_diff
```

```{r}
max(nyc_stations_15M_interval_diff[-1])
```

```{r}
nyc_stations_15M_interval_diff_long <- nyc_stations_15M_interval_diff %>%
  pivot_longer(!timestamp, names_to = "station.id", values_to = "interval.diff")
```

```{r}
nyc_stations_15M_interval_diff_long
```
...
The above 108 stations indicate rebalancing occurred 8 or more time in the span of the 2 weeks

Rebalancing defined as the interval increase of 12 or bikes from one 15min interval to the next

```{r}
stations_attrs_trim %>%
  filter(station.id == "477")
```
