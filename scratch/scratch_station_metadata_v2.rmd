...
plot(test$bikes.avail.count,type = "b",col="red")
lines(round(pred),col="blue")
```
...
```{r}
library(forecast)
stations_data <- stations_bike_avail_by_time_no_na_group %>%
  filter(cluster == "1") %>%
  dplyr::select(timestamp, bikes.avail.count)

citibike_ts <- stations_data %>%
  mutate(timestamp = as_datetime(timestamp)) %>%
  as_tsibble(index=timestamp)

citibike_ts_miss <- citibike_ts %>%
  fill_gaps()

citibike_ts_fill <- citibike_ts_miss %>%
  # Fit ARIMA model to the data containing missing values
  model(ARIMA(bikes.avail.count)) %>%
  # Estimate Trips for all periods
  interpolate(citibike_ts_miss)

```

#citibike_ts2 <- na.interp(citibike_ts)

# I think it doesn't like that my time is every 15 minutes instead of every minute
...
```{r}
autoplot(citibike_ts, bikes.avail.count) +
