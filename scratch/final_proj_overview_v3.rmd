...
subtitle: "DATA 698 Research Project: v3 11/26 5:45P"
...
We examine the bike trip data from August through October 2022 and the bike availability counts by docking station for two weeks in November to identify the patterns of use and distinguishing factors to bike availability and in doing so reveal the challenges of operational rebalancing and frequency of zero bike availability. In order to construct a real-world, user-friendly model, we apply two modeling techniques to the bike availability data. First, we cluster the Brooklyn docking stations to establish quarter-mile based clusters informed by acceptable walking distance. Second, we apply count data regression models to predict bike availability counts by cluster. The selected algorithm is a Poisson-based model with a comparable RMSE to the other attempted models and the lowest count of missed zero predictions. Based on the selected count data regression model of clustered docking stations, a working proof of concept was created to provide true bike availability predictions for location, day, and time.
...
2. Identify system rebalancing and its impact on the level of availability at a given station or cluster of stations.
...
Based on the selected count data regression model of clustered docking stations, a working proof of concept was created to provide true bike availability predictions for location, day, and time.
...
This project relies on two primary data sources from Citi Bike. The first dataset, the complete set of bike trips for the three months spanning August-October 2022 is used to identify the patterns of bike use across New York City. The second dataset, an aggregation of each docking station status every 15 minutes for the two-week period of Monday, Oct. 31, 2022 through Sunday, Nov. 13, 2022. The station status API provides the real-time bike availability count of every docking station in the Citi Bike system. This two-week dataset provides an accurate depiction of the available bikes, intentionally accounting for operational rebalancing.

Citi Bike provides individual bike trip data on a monthly basis available at https://ride.citibikenyc.com/system-data. This project uses three months of NYC-scoped bike trip data from August through October 2022. A note on the system data webpage indicates trips taken by staff to service or inspect the system have been removed from the dataset. Also, any trips below 60 seconds have also been omitted. With this pruning by the system maintainers, the remaining trips are considered to be valid bike trips. The dataset contains 13 variables for each bike trip originating at a NYC-based docking station.
...
The NYC Open Data (free public data published by New York City agencies and partners) provides a GeoJSON file for the polygons defining each neighborhood in NYC according to the 2010 Neighborhood Tabulation Areas (NTAs). Each NTA is associated with one of the five NYC boroughs.
...
We faced three primary challenges during this research project. First, the large volume of data proved overwhelming for the computing processors used. Given the count of over 10 million trips and over 1,700 docking stations in NYC, the large data provided a base for several interesting data visualizations but pushed our computing powers to their system limits. The computing power limits ultimately informed the decision to focus the prediction model on the borough of Brooklyn instead of the entirety of NYC.
...
As seen in the visual "Median Number of Bike Trips," the member user counts show rush-hour spikes on weekdays whereas casual users do show higher counts around 5pm and 6pm on weekdays. The comparison of the two user groups also points to greater usage overall by members. For every day of the week, the average member counts per hour are greater than the casual users. On weekend days, both user groups show a pattern indicative of recreational use with a plateau of use during the middle of the day and without the distinct peaks found on the weekdays. Also, the higher usage of by member users throughout the middle of the day may indicate even if someone is a member the primary reason may not be transportation to and from work.
...
**Brooklyn and Queens:** In less dramatic fashion as Manhattan, the 8 a.m. panel shows movement from the outlying areas in read toward the East River and subway station locations in blue. The evening panels at 5 p.m. and 8 p.m. capture the inverse of the morning movement with more blue docking stations in the outlying areas as bikes create an surplus in the residential areas.

### Surplus by Borough

Given the visible distinctions on the map of where bike surpluses and shortages occur across New York City, we evaluate the shortage and surplus by borough to understand bike availability at a more granular level.
...
The plot "Station Surplus by Borough Histogram" denotes a near normal distribution for the four boroughs included in the dataset --- the Bronx, Brooklyn, Manhattan, and Queens. The balanced distribution across the four NYC boroughs likely demonstrates that bike trips are contained within a borough. With average duration less than 10 minutes for member users and less than 20 minutes for casual users, the distance traveled for all bike trips is likely less than three miles. The 30-minute base rental rate dictates a limited travel distance inhibiting users from traveling across boroughs via the bikeshare system.
...
p_rebalance_station
```
According to the Citi Bike operating report for September 2022 (latest available as of report submission), the Citi Bike staff rebalanced a total of 70,117 bikes during the month. The operating report calculates an average bike fleet in September as 24,514.0 with 24,327 bikes in the fleet on the last day of the month. Given the total number of rebalanced bike over 70,000 for the month and an average of 24,514 bikes available per day, then almost 10% of bikes are rebalanced daily. As seen in the maps of surplus and shortage by docking station, an even distribution of bikes does not occur and must be addressed by the operators to ensure daily bike availability.
...
To predict bike availability counts by cluster, we apply count data regression models. We selected the Generalized Linear Models based on Poisson, Quasi-Poisson, Negative Binomial and Zero-Inflated algorithms. The availability data meets most of the Poisson regression criteria. Response variable is a count per unit. The observations are independent. Unfortunately, the variance is greater than the mean, which indicates overdispersion in the data. The mean of the bikes available by cluster is 36.34474 and the population variance over the dataset is 908.1704 indicating overdispersion exists. Due to overdispersion, we selected Negative Binomial and Quasi-Poisson as base algorithms in addition to Poisson. The Zero-Inflated algorithm was chosen based on the previous observation in which zero bike availability is the most common value across all the docking-station clusters.

For evaluating the models, the two full weeks of bike availability data spanning Oct. 31 through Nov. 13 are partitioned into a training set of 75% and a test set of 25%. Given the common, yet inconsistent occurrences of rebalancing identified previously, we decided to not impute the instances of rebalancing, but instead keep the data accurate in an attempt at a more valuable model. The dataset follows a time series pattern but in order to predict a specific day and time of the week agnostic of date, regression models for count data are used. 

The model inputs mimic a map-based application in which a user selects a point on a map along with the type of day and time to receive an estimation of the bikes available at that future date, time, and location. Based on the user-provided longitude and latitude, a nearest neighbor search is performed to identify the nearest docking station. The translation of the longitude and latitude to the corresponding cluster of the nearest-neighbor docking station would be the input to the model. The categorical variable type of day, weekday or weekend, was chosen due to the differing usage patterns on weekdays and weekends. Within each category, weekday and weekend, the similarities allowed us to simplify the model to just two values. The time of day is converted to the start of the hour. Models were constructed using the 15-minute intervals, due to computing system constraints the time interval of one hour was selected. The dependent variable is the count of available bikes with the floor function applied to ensure a whole number prediction.
...
Due to the high number of clusters and hourly intervals used for the regression models, the models created a large volume of dummy variables. For variable cluster, 212 variables are created, for variable time, 23 variables created, and for day one additional variable is created. This variable expansion makes the model variables difficult to assess individually as the model only has three input categorical variables.

The results of the first three models identical results of deviance and degrees of freedom for Poison and Quasi-Poisson as expected. The Negative Binomial model did produce a lower AIC than the Poisson model indicating a slightly better model.
...
The plots of the residuals vs. the fitted values for Poisson, Quasi-Poisson, and Negative Binomial each indicate heteroscedasticity. The evidence of heteroscedasticity shows increasing variance as the fitted values increase. The Poisson and Quasi-Poisson appear identical while the Negative Binomial plot is quite similar.
...
In the end we selected the Poisson model because it resulted in the second best RMSE and MAE and had the fewest `Missed.Zero` counts despite the variance being greater than the mean. The Poisson regression equation is presented for the selected model.
