...
Our methodology for answering these questions follows the steps listed below: Data Collection & Preprocessing, Data Exploration & Analysis, Complete Algorithm Approach, Hierarchical Clustering and Count Data Regression Modeling. The nature of each is described below:
...
**Algorithm Approach:** Outline the modeling techniques required to properly group the Brooklyn docking stations and then construct a proper regression model to predict the bike availability for each docking-station cluster.
...
The station information API, also available from Citi Bike, provided the following pertinent variables for each docking station in the sytem.
...
XX making a data table of the XXX API calls over the course of the two weeks, and summing the available bikes and then joining that with station information to provide a capacity cap on the availability numbers.
...
```{r display.surplus.by.station, echo=F, message=F, warning=F, include=T, fig.height=4.5, fig.asp=1}
img <- readJPEG("stations_surplus_sum.jpg")
plot(1:10,ty="n", axes = 0, xlab='', ylab='', main='Overall Surplus/Shortage by Docking Station')
...
\begin{figure}[H]
  ...
  \caption{Weekday Pattern: Average Wednesday}
\end{figure}
...
- Predict count of bikes available per cluster


### Zero Bike Availability

The bike availability prediction model relied on the following scope of docking station availability information.

\begin{myitemize}
    \item \textbf{Timeframe:} Oct. 31 - Nov. 13 of 2022 (14 days)
    \item \textbf{Time interval:} Every hour
...
\begin{center}
  \includegraphics[width=0.8\textwidth, keepaspectratio,trim={5cm 4cm 0cm 0cm},clip]{bk_stations_zero_avail.jpg}
\end{center}
...
\begin{center}
  \includegraphics[width=0.8\textwidth, keepaspectratio,trim={6cm 4cm 6cm 4cm},clip]{bk_stations_by_cluster.jpg}
\end{center}
...
```{r display.bk.freqOfZero.by.cluster, echo=F, message=F, warning=F, include=T, fig.height=5, cache=T}
